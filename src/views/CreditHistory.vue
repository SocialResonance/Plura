<template>
	<div class="plura-credit-history">
		...
		<div v-if="!loading" class="transaction-history">
			...
			<div v-if="transactions.length > 0" class="pagination">
				<button :disabled="offset === 0" @click="loadPreviousPage">
					{{ t('plura', 'Previous') }}
				</button>
				<span>{{ t('plura', 'Page {page}', { page: Math.floor(offset / limit) + 1 }) }}</span>
				<button :disabled="transactions.length < limit" @click="loadNextPage">
					{{ t('plura', 'Next') }}
				</button>
			</div>
		</div>
		...
		<div v-if="selectedTransaction" class="transaction-details">
			...
			<div class="transaction-details-content">
				...
				<div v-if="selectedTransaction.related_entity_id" class="transaction-detail-row">
					<label>{{ t('plura', 'Related Entity ID') }}</label>
					<div class="transaction-related-entity">
			...
			<div class="modal-buttons">
				<button class="primary" @click="selectedTransaction = null">
					{{ t('plura', 'Close') }}
				</button>
			</div>
		</div>
